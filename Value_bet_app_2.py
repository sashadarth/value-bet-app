...
# 🔴 CAMBIA LA PRIMA VOLTA: metti la tua API key di API-Football
...
# --- Lista leghe con ID API-Football ---
lega_id = {
    "Serie A": 135,
    "Premier League": 39,
    "La Liga": 140,
    ...
    "Champions League": 2,
    "Europa League": 3,
    "Conference League": 3,  # esempio, aggiornare con ID reale
    "FA Cup": 45,
    "Coppa Italia": 41,
    "Supercoppa": 42,
    "Coppa Spagnola": 43
...
def get_teams(lega_codice):
    url = f"https://v3.football.api-sports.io/teams?league={lega_codice}&season=2025"
    headers = {
    ...
    try:
        ...
        if not teams:
            st.error("Nessuna squadra trovata per questa lega")
            st.stop()
        return teams
...
if st.button("Calcola Value Bet"):
    dati_matematici = []
    for mercato in mercati:
        prob = 0.5  # esempio di probabilità automatica, puoi sostituire con calcolo reale
